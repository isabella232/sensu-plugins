...
if(!defined $options{C} || !defined $options{H})
{
        die("Usage: check-dell-md.pl -C SMcliPath -H host");
}

my $hosts = join(" ", split(",", $options{H}));
my $smcli = $options{C};

#Execute SMcli command
my $cmd = "$smcli $hosts -S -quick -c \"show storageArray healthStatus;\";";
if(!open(PIPE, "$cmd|"))
{
